<div>
    <div>
                ...
                @foreach ($vacantes as $vacante)
                    <div class="space-y-2 p-5 border border-gray-200 md:flex md:justify-between">
                        ...
                        <div class="flex flex-col md:p-0  items-stretch gap-2 md:flex-row md:items-center  mt-3 ">
                            ...
                            <button
                                class="bg-red-600 py-2 px-4 rounded-lg text-xs font-bold uppercase text-center text-white"
        ...
        @else
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <p class="p-5 text-gray-600 font-semibold text-sm text-center">Aún no has publicado ninguna vacante</p>
            </div>
        @endif
    </div>
        ...
        Livewire.on('mostrarAlerta', ({ vacanteId }) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire(
                        'Eliminado!',
                        'La vacante ha sido elimanda correctamente',
                        'success'
                    )
                }
            })
